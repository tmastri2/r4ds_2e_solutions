...
Formal introduction to annotations is later in this chapter.
...
-   If you set color and shape to the same variable, ggplot will automatically make a single legend. If you choose 2 different variables, then it will make 2 legends to accommodate all the combinations.

-   You should note some values overlap in the plot, an improvement you can do is to jitter the values to make sure all values are appropriately visualized.

    -   Jitter function explained in this [r4ds section:](https://r4ds.hadley.nz/layers#position-adjustments) from chapter 9.

#### 3.

Made a simple plot showing off the Lahman package in case you're a baseball stats nerd, but make your own plot.
...
-   Can read more about the nuances of hjust/vjust and other aesthetics by using the command: `vignette("ggplot2-specs")`
...
By default, if you facet after a geom_text call it will apply to all plots.
...
Can read the documentation for the geom_label() function for this information and more.
...
You need to use scale_fill_gradient in order to color in the plot. If you want to change the color of the plot using the geom_hex arguments, you want to use color, since the fill argument removes any gradient from the hexagons.
...
The first argument for a scale is the name, which gets used as the axis or legend title. (To read the documentation for a function, use the command `?name_of_function`)
...
Make your own plot, decided to make 2 simple plots to show how a white graph background looks on a white webpage. (As influenced by footnote #2 in this chapter which explains why they chose a gray background for the default graph).
...
It's an order of operations issue, since `/` gets evaluated before `+` or `|`.
